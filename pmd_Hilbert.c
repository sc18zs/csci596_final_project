...
systems using the Message Passing Interface (MPI) standard, with
Hilbert curve reordering for improved cache performance.
----------------------------------------------------------------------*/
#include "pmd.h"
#include <math.h>
#include <stdlib.h>
...
/*--------------------------------------------------------------------*/
  ...
  for (stepCount = 1; stepCount <= StepLimit; stepCount++) {
    single_step();
    if (stepCount % REORDER_INTERVAL == 0) {
      reorderAtoms();
    }
...
/*----------------------------------------------------------------------
Comparison function for qsort to sort atoms by Hilbert index.
----------------------------------------------------------------------*/
  unsigned int indexA = ((struct Atom *)a)->hilbertIndex;
  unsigned int indexB = ((struct Atom *)b)->hilbertIndex;
